...
Showmessage('Please Save The Project Before Running.');
end;
...
if  FilePath = '' then
begin
Showmessage('Please Save The Project Before Building.');
end else
...
showmessage(' The project was saved successfully');
...
begin
Showmessage('Can Not Run The Program . Not Found "\Compiler\Nova.exe" ');
...
begin
if  not fileexists(extractfilepath( application.ExeName) + '\Help\Doc.chm') then
Showmessage( 'Help File Not Found ... ' +#13 + extractfilepath( application.ExeName) + '\Help\Doc.chm' )
else
ShellExecute(Handle, 'open', PChar('hh'), pchar(extractfilepath( application.ExeName) + '\Help\Doc.chm'), nil, SW_SHOW);
end;
